    ...
    try:
        ...
        for circuit in circuits:
            ...
            if len(base_scores) > 2:
                score_next_1 = base_scores[1]
                score_next_2 = base_scores[2]
            elif len(base_scores) > 1: 
                score_next_1 = base_scores[1]
            score_cnt = 1
            for score in base_scores:
                ...
                if score_n > 0:
                    ...
                    rec_cnt = 1
                    while True:  
                        if score_n_1 > 10 or score_n_2 > 10:
                            score_n_1 =  (score_n_1*0.5 if score_n_1 > 10 else 0 ) + (score_n_2*0.25 if score_n_2 > 10 else 0)
                            score_n_2 = score_n
                            score_n = score_n_1 
                            time_2 = score[2] + (rec_cnt - 1)*3600                
                            time_1 = score[2] + rec_cnt*3600                           
                            if ((score_cnt + rec_cnt) <= len(base_scores) and (score[2] + 3600*rec_cnt) != base_scores[score_cnt + rec_cnt - 1][2])  or (score_cnt + rec_cnt) > len(base_scores):
                                if score_n_1 >= 10:
                                    final_scores.append(
                                        {
                                            'monitor_id':circuit[0],
